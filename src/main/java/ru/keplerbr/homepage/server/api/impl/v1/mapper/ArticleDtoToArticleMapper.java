...
public abstract class ArticleDtoToArticleMapper {
  ...
  private final Map<String, BiConsumer<ArticleDto, Article>> setters;

  public ArticleDtoToArticleMapper() {
    setters = new HashMap<>() {{
      ...
      put("url", (dto, article) -> dto.setUrl(article.getUrl()));
    }};
  }
  ...
  @Mapping(target = "language", source = "request.language")
  @Mapping(target = "tags", source = "request.tags")
  public abstract Article dtoToArticle(ArticleDto request);
  ...
  public ArticleDto articleToDto(Article article, Set<String> includeFields) {
    ...
    includeFields.forEach((fieldName) -> {
      if (setters.containsKey(fieldName)) {
        setters.get(fieldName).accept(articleDto, article);
      }
    });
